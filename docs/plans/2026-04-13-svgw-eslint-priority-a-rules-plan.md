...
```bash
cd /opt/SeoulVentures/regle/SeoulVenturesGroupware
grep -n 'ClientNameComparator\|TRIM_RE\|\\\\s\|White_Space' app/Support/Review/ClientNameComparator.php 2>/dev/null
```

결과 확인. PHP 가 `\s`, `\p{White_Space}`, 혹은 `[\s\p{White_Space}]` 중 어느 표기를 쓰는지 기록. 구현 PR 본문에 "백엔드 grep 결과: `<출력>`" 으로 첨부.
...
파일에서 `setup(_, { slots }) {` 바로 다음 줄에 빈 줄 + 헬퍼 + 빈 줄 삽입. 기존 `onEnter = (element: HTMLElement) => {` 직전.
...
```bash
cd /opt/SeoulVentures/regle/SeoulVenturesGroupware
php artisan tinker --execute='echo \App\Models\User::where("email", "regexp", "@\[")->count();'
```
...
파일 내 `tsRules` 상수 내부에서 다음 2줄 삭제:

```js
  // ts-eslint 7→8 recommended 에서 신규 활성화. 기존 코드 3건 존재.
